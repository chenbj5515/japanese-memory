...
export function getRandomItemsFromArray(objects: StudyObject[]): StudyObject[] {
  ...
  const timeRanges: [number, number][] = [[180, 360], [90, 180], [30, 90], [0, 30]];  // days ago
  ...
  const enhancedObjects = objects.map(obj => ({
      ...obj,
      daysAgo: (Date.now() - new Date(obj.create_time).getTime()) / (1000 * 60 * 60 * 24)
  }));

  // Sort without mutating the original array.
  const sortedObjects = [...enhancedObjects].sort((a, b) => 
  ...
  const filterAndSlice = (objects: StudyObject[], reviewTimes: number, start: number, end: number, toSelect: number) =>
      objects.filter(obj => obj.review_times === reviewTimes && start <= obj.daysAgo! && obj.daysAgo! < end)
             .slice(0, toSelect);
  ...
  if (remaining > 0) {
      const nonSelectedObjects = sortedObjects.filter(obj => !selected.find(sel => sel.id === obj.id));
      timeRanges.forEach(([start, end]) => {
  ...
  if (selected.length < desiredSelections) {
      const additionalItems = sortedObjects.filter(obj => !selected.find(sel => sel.id === obj.id));
      selected = [
          ...selected,
